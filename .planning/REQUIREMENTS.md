...
- [ ] **EXPORT-01**: ExportRecord Prisma model persists export history (report type, format, file size, R2 key, download URL, generated by, status)
- [ ] **EXPORT-02**: ExportsModule backend infrastructure — service dispatches to builders, controller exposes generate + history endpoints
- [ ] **EXPORT-03**: StorageService can upload server-generated files (Buffer) directly to R2 without MIME whitelist restriction
