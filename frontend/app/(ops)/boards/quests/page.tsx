...
export default function QuestBoardPage() {
  ...
  const isEmpty = !questsLoading && !questsError && filtered.length === 0;
  const hasFilters = missionFilter !== 'all' || assigneeFilter !== 'all';
  ...
  return (
    <div className="space-y-6">
      ...
      {questsLoading && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="rounded-lg bg-muted/30 p-4 space-y-3 animate-pulse motion-reduce:animate-none"
            >
      ...
      {questsError && (
        <div className="space-y-3">
          <Alert variant="destructive">
            ...
            <AlertDescription>
              Couldn&apos;t load the quest board. Try again in a moment.
            </AlertDescription>
          </Alert>
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              {hasFilters
                ...
                : 'Quests appear here once a mission has been broken down into weekly quests.'}
            </p>
          </div>
          {hasFilters ? (
            <Button
      ...
      {/* Kanban columns */}
      {!questsLoading && !questsError && filtered.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <QuestKanbanColumn
            title="Not Started"
            quests={notStarted}
          ...
          <QuestKanbanColumn
            title="Completed"
            quests={completed}
            accentClass="border-l-[var(--status-good)]"
          />
        </div>
      )}
    </div>
