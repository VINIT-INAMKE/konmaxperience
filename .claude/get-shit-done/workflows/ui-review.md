...
```bash
UI_AUDITOR_MODEL=$(node "D:/aditee/konmaxperience/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-ui-auditor --raw)
```
...
```markdown
Read D:/aditee/konmaxperience/.claude/agents/gsd-ui-auditor.md for instructions.
...
```bash
node "D:/aditee/konmaxperience/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): UI audit review" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-REVIEW.md"
```
