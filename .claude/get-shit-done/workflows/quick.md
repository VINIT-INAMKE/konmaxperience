...
```bash
INIT=$(node "D:/aditee/konmaxperience/.claude/get-shit-done/bin/gsd-tools.cjs" init quick "$DESCRIPTION")
...
```bash
node "D:/aditee/konmaxperience/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
