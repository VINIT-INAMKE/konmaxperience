...
- **Global scope**: `D:/aditee/konmaxperience/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/aditee/konmaxperience/.claude/notes/`)
...
2. Glob `D:/aditee/konmaxperience/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/aditee/konmaxperience/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/aditee/konmaxperience/.claude/notes/` — works in any directory
