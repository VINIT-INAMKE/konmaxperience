...
```bash
TRANSITION=$(node "D:/aditee/konmaxperience/.claude/get-shit-done/bin/gsd-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "D:/aditee/konmaxperience/.claude/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
node "D:/aditee/konmaxperience/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active false
```
